...
class NormalApproximation(object):
    def __init__(self, mu_init=lasagne.init.Normal(1), rho_init=lasagne.init.Constant(0)):
    ...
    def __call__(self, layer, spec, shape, **tags):
        ...
        rho = layer.add_param(rho_spec, shape, name='bnn.rho',rho=True, **tags)
        mean = layer.add_param(mu_spec, shape, name='bnn.mu',mu=True, **tags)
...
def sample_output(output_layer,input_dict,
    ...
    out_ravel = lasagne.layers.get_output(output_layer,repeated_input_dict,**kwargs)
    
    return aggregate(out_ravel.reshape((out_ravel.shape[0]//n_samples,n_samples,)+tuple(out_ravel.shape[1:])))
