...
def extract_params(bnn_weights,pred_loss,delta=0.01):
    rhos = filter(lambda p: p.name.endswith('bnn.rho'), bnn_weights)
    mus = filter(lambda p: p.name.endswith('bnn.mu'), bnn_weights)
    ...
    for i in xrange(len(new_mus)):
        new_mus[i].name = 'new_mu'
    for i in xrange(len(new_rhos)):
